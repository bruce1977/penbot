...
> 知识库管线（`workflow-knowledge.md`）中：`gatherer` 可使用 `wechat-mp-gather` 采集公众号文章到 `inbox/`。文档分析由 `coordinator` 调用 `skills/knowledge/scripts/analyze_start.js` 脚本完成（元数据提取+评分+合并），不依赖 LLM Agent。
