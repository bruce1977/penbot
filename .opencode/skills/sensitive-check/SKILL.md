...
description: 敏感词审查（sensitive-lexicon-mcp），覆盖15分类67038词
---
...
- `sensitive-lexicon_detect_sensitive_words`: 检测文本敏感词
- `sensitive-lexicon_filter_sensitive_words`: 过滤敏感词
- `sensitive-lexicon_get_categories`: 获取敏感词分类
- `sensitive-lexicon_get_word_count`: 获取词库数量
...
1. 获取待检查文本
2. 调用 `sensitive-lexicon_detect_sensitive_words` 检测
3. 按风险等级分级（高危/中危/低危）
4. 输出结构化报告
